...
export let infoboxImageUrl = wId =>
	Promise.all([
		...
		new Promise(resolve =>
			...
			fetchJsonp(
				"https://fr.wikipedia.org/w/api.php?action=parse&format=json&prop=text&redirects=true&page=" + wId
			)
				.then(res => res.json())
				.then( ({parse: {text}}) => {
					let html = text['*'],
						...
						originalImageUrl = 'https://' + (url.indexOf('thumb') > -1 ?
							url.replace('/thumb', '').split('/').slice(0, -1).join('/')
							: url)
		...
		new Promise(resolve =>
			fetchJsonp(
				"https://fr.wikipedia.org/w/api.php?action=query&prop=extracts&format=json&exintro=&rawcontinue=&redirects=true&titles=" + wId
			)
				.then(res => res.json())
				.then( ({query: {pages}}) => {
...
export let cacheWikiPages = nodes =>
	Promise.all(
		nodes
			...
			.map(({id}) => infoboxImageUrl(id.replace('w:', '')).then(
				res => window.wikiCache[id] = res
			))
	)
