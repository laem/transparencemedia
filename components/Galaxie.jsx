...
class Galaxie extends React.Component {
  render() {
    return (
      <div id="galaxie">
        <div
          style={{
            marginLeft: this.props.width * (1 / 6),
  ...
  componentDidMount() {
    let { journal } = this.props,
      ...
      { nodes, links } = items
        ...
        .reduce(
          ({ nodes, links }, { nodes: n, links: l }) => ({ nodes: { ...nodes, ...n }, links: [...links, ...l] }),
          { nodes: {}, links: [] },
        )
    ...
    cacheWikiPages(R.values(nodes)).then(() => {
      let data = {
          ...
          edges: R.uniq(links).map(({ source, target, value }) => ({
            from: source,
            to: target,
            arrows: "to",
...
export default windowSize(Galaxie)
